...
def generate_data(df, one_hot, n_samples, LATENT_DIM=8, EPOCHS=50, BATCH_SIZE=32, kl_weight=0.001):
    ...
    for group in one_hot:
        ...
        probs = generated_df[group].values
        ...
        selected_idx = [np.random.choice(probs.shape[1], p=probs[i]) for i in rows]

        arr = np.zeros_like(probs)
        arr[rows, selected_idx] = 1
        generated_df[group] = arr.astype(int)
